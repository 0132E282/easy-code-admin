...
// MultiSelect variants styles with cva
const multiSelectVariants = cva('transition-all ease-in-out m-1 duration-300 ', {
  variants: {
    variant: {
      default: 'bg-card text-foreground border-foreground/10 hover:bg-card/80',
      secondary: 'bg-secondary text-secondary-foreground border-foreground/10 hover:bg-secondary/80',
  ...
  ({ options, onValueChange, variant, defaultValue = [], placeholder = 'Select options', animation = 0, maxCount = 3, modalPopover = false, asChild = false, className, ...props }, ref) => {
    ...
    return (
      <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
        ...
        <PopoverContent className='w-[500px] p-0' align='start' onEscapeKeyDown={() => setIsPopoverOpen(false)}>
          <Command>
            <CommandInput placeholder='Search...' onKeyDown={handleInputKeyDown} className='w-full p-2 border-0 rounded-lg focus:ring-0 outline-none' />
